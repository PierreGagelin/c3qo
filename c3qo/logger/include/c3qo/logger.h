...
#define LOGGER_ERR(msg, ...)     if (logger_level >= LOGGER_LEVEL_ERR)     { syslog(LOG_ERR,     "[ERR] "     msg " (%s:%d)", ##__VA_ARGS__, __func__, __LINE__); }
#define LOGGER_WARNING(msg, ...) if (logger_level >= LOGGER_LEVEL_WARNING) { syslog(LOG_WARNING, "[WARNING] " msg " (%s:%d)", ##__VA_ARGS__, __func__, __LINE__); }
#define LOGGER_NOTICE(msg, ...)  if (logger_level >= LOGGER_LEVEL_NOTICE)  { syslog(LOG_NOTICE,  "[NOTICE] "  msg " (%s:%d)", ##__VA_ARGS__, __func__, __LINE__); }
