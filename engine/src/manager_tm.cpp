...
//
bool manager::timer_add(const struct timer &tm)
{
    struct timer tmr;
    ...
    if (tm.bk == nullptr)
    {
        LOGGER_ERR("Cannot add timer: nullptr block");
        return false;
    }

    // Copy timer information
    tmr = tm;
